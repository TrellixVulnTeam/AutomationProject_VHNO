...
class Camera_Page(System):
    ...
    def start_camera(self):
        logger.info("function:" + sys._getframe().f_code.co_name + ":启动camera app:")
        self.device.start_app("com.tcl.camera")
        sleep(1)

    def stop_camera(self):
        logger.info("function:" + sys._getframe().f_code.co_name + ":关闭camera app:")
        sleep(1)
        self.device.stop_app("com.tcl.camera")

    def enter_camera_settings(self):
        logger.info("function:" + sys._getframe().f_code.co_name + ":进入camera设置界面:")
        camera_settings = self.top_function_bar.wait().children()[0]
        camera_settings.click()

    def change_ai_scene_status(self):
        logger.info("function:" + sys._getframe().f_code.co_name + ":改变Ai scene status的值:")
